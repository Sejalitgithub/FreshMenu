...
class FirstFood extends StatefulWidget {
  const FirstFood({Key? key}) : super(key: key);
...
class _FirstFoodState extends State<FirstFood> {
  int itemCount = 0; // State variable to track the item count
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomNavigationBar: buildViewCartButton(),
    );
  }
  ...
  Widget buildViewCartButton() {
    return Padding(
      ...
      child: ElevatedButton(
        onPressed: () {
          // Action to perform when the "View Cart" button is pressed
        },
